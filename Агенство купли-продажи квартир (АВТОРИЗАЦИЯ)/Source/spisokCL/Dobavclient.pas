...
begin                //выбераем значения которые хотим вводить в таблице
  IBQuery1.SQL.Clear;
  IBQuery1.SQL.Add('insert into CUSTOMERS(id_custimers, firstname, lastname, patronymic, tel_customers, passport, inn) values');
  IBQuery1.SQL.Add('('''+kodpok.text+''','''+impok.text+''','''+fampok.text+''','''+otpok.text+''','''+telpok.text+''','''+paspok.text+''','''+innpok.text+''')');
  IBQuery1.ExecSQL;
  IBQuery1.Active := false;
  IBQuery1.SQL.Clear;
